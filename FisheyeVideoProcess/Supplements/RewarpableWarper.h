...
public:
	void setScale(float _s) {projector_.scale = _s;}
	void setProjectorData(std::vector<std::vector<float>> &d) {
		projector_.setAllMatsMultiple(d);
	}
	std::vector<std::vector<float>> getProjectorAllData() {
		return projector_.getAllMatsMultiple();
	}
};
